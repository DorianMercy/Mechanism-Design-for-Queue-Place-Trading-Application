# potential_pairs lists each potential pair and its reduced time cost
def match(potential_pairs):
        ...
        if n not in bad_index:
            for p in range(n+1,len(comb_rule1)):
                ...
                if diff == {0,-1}:
                    ...
                    bad_index.append(p)
    # find the combination(s) that most reduce costs
    most_redu = 0
    result = []
    for r in comb_rule2:
        redu = 0
    ...
    return result
# demo
# 'a', 'b', 'c', 'd' are queuers. The numbers are reduced time costs.
